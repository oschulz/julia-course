...
using Statistics, StatsBase, Distributions, ValueShapes, InverseFunctions
using DensityInterface, BAT
using AutoDiffOperators
import ForwardDiff, AdvancedHMC
...
likelihood = let data = (x = sel_data.xy.x, y = sel_data.xy.y, t = sel_data.t)
    logfuncdensity(function (v)
        ...
        sum(logpdf.(Normal.(sim_data.x, σ_x), data.x)) + sum(logpdf.(Normal.(sim_data.y, σ_y), data.y))
    end)
end
...
@benchmark logdensityof(posterior)(rand(prior))
...
ad = get_batcontext().ad

tpstr, trafo = bat_transform(PriorToGaussian(), posterior)
v = rand(prior)
x = trafo(v)

ldf = logdensityof(tpstr)

ldf(x)
ForwardDiff.gradient(ldf, x)
with_gradient(ldf, x, ad)
gradient_func(ldf, ad)(x)

@inferred ldf(x)
@inferred ForwardDiff.gradient(ldf, x)
@inferred with_gradient(ldf, x, ad)
@inferred gradient_func(ldf, ad)(x)

@benchmark $ldf($x)
@benchmark ForwardDiff.gradient($ldf, $x)
@benchmark with_gradient($ldf, $x, ad)
@benchmark $(gradient_func(ldf, ad))($x)
